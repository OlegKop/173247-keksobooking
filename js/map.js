...
var getPropertyKeks = function (avatar, title, priceMin, priceMax, type, roomsMax, 
  guestsMax, checkInOut, features, photos, xMin, xMax, yMin, yMax) {
    for (var i = 0; i <= 8; i++) {
      ...
      for (var j = 0; j <= i; j++){
        ...
        var titleList = title;
        var checkInOutR = nameArr(checkInOut);
        var roomTypeR = nameArr(type);
        var featureR = featureRand(features);
        var photoR =  mixArray(photos); 
...
var getMapPin = function (pinListElement) {
...
var getFeaturs = function (featurArray, element) {
  var childElement = element.querySelectorAll('li');
   for (var i = featurArray.length; i < featureType.length; i++) {
...
var getMapCard = function (pinListCard) {
...
var insertedpinListCard = pinListCard.insertBefore(fragmentCard, mapFilters);
